...
def test_qwen3_args_from_torchtitan_config(flavor, expected):
    ...
    try:
        from torchtitan.models.qwen3 import (
            qwen3_configs,  # type: ignore[import-not-found]
        )
    except Exception as exc:
        pytest.skip(f"torchtitan Qwen3 config unavailable: {exc}")

    args = qwen3_args_from_torchtitan_config(qwen3_configs[flavor](attn_backend="sdpa"))
...
def test_qwen3_cos_sin_rope_matches_torchtitan_helper():
    ...
    try:
        ...
        from torchtitan.models.common.rope import (
            apply_rotary_emb_cos_sin as tt_apply_rotary_emb_cos_sin,  # type: ignore[import-not-found]
        )
    except Exception as exc:
