    ...
    with FakeTensorMode(allow_non_fake_inputs=True) as fake_mode:
        ...
        for node in fwd_graph.graph.nodes:
            if node.op == 'placeholder':
                ...
                sum_node = joint_graph.call_function(torch.ops.aten.sum.default, (output_node,))
                sum_node.meta["val"] = torch.tensor(1.0)

                # Add backward nodes
                bw_node = joint_graph.call_function(torch.ops.aten.mul.Tensor, (sum_node, 1.0))
                ...
                tangent_node.meta["val"] = output_node.meta.get("val", torch.randn(2, 8, 64))
...
class TestACPolicy:
    ...
    def test_apply_ac_policy(self, llama3_model):
        ...
        expected_saved_nodes = (total_mm_nodes + 1) // 2  # ceiling division for odd counts

        # Verify the every-other-node policy is working correctly
        assert len(marked_nodes_to_save) == expected_saved_nodes, f"Expected {expected_saved_nodes} nodes to be saved, but got {len(marked_nodes_to_save)}"