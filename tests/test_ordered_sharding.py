...
# @patch("torch.cuda.device_count", lambda: 8)
# @patch("torch.cuda.get_device_name", lambda device: "H100")
# def test_compute_optimal_placement_order_mixed_grad_requirements():
#     """Test that the function correctly processes mixed trainable/non-trainable parameters."""
...
#             self.trainable_linear = nn.Linear(dim, dim, bias=False)  # trainable
#             self.non_trainable_linear = nn.Linear(dim, dim, bias=False)  # will set to non-trainable

#             # Set one to non-trainable
#             self.non_trainable_linear.weight.requires_grad = False

#         def forward(self, x):
#             return self.non_trainable_linear(self.trainable_linear(x))

#     def model_fn():
#         return MixedModel()

#     def input_fn():
#         return torch.randn(128, dim, device=device, requires_grad=True)

#     # Create 1D mesh for simpler test case
#     world_size = torch.distributed.get_world_size()
#     device_mesh_1d = torch.distributed.device_mesh.init_device_mesh(
...
#         # Should handle mixed case without issues
#         placement_order = compute_optimal_placement_order_for_parameters(
#             autop.gm, sharding_placement
#         )
...
#         # At least one should have a valid grad (the trainable param)
#         has_valid_grad = any(grad is not None for param, grad in param_and_grad_nodes)
#         assert has_valid_grad, "Expected at least one parameter to have a valid gradient"
