...
def _to_activation_device(
    tensor: torch.Tensor, activation: torch.Tensor
) -> torch.Tensor:
...
def _rms_norm(x: torch.Tensor, norm: nn.RMSNorm) -> torch.Tensor:
    weight = _to_activation_device(norm.weight, x) if norm.weight is not None else None
...
def _linear(x: torch.Tensor, linear: nn.Linear) -> torch.Tensor:
    ...
    bias = _to_activation_device(linear.bias, x) if linear.bias is not None else None
...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        expert_bias = _to_activation_device(self.expert_bias, scores)  # type: ignore[arg-type]
        scores_for_choice = (
            scores + expert_bias if self.load_balance_coeff is not None else scores
        )
...
class Transformer(nn.Module):
    ...
    def init_weights(
        ...
        buffer_device = buffer_device or self.freqs_cos_sin.device  # type: ignore[has-type,assignment]
