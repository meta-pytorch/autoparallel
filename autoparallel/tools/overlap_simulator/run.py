...
from torch import device, tensor  # noqa: F401 (used by repro modules)
from torch._dynamo.testing import rand_strided  # noqa: F401 (used by repro modules)
from torch.fx.operator_schemas import normalize_function

from autoparallel.graph_passes.debug_helpers import create_execution_trace

# Constants
DEFAULT_VARIANT = "llama3_8b_bw_256_2d"
...
class Stats:
    ...
    def __str__(self) -> str:
        return (
            f"AG:{self.num_ag}, RS:{self.num_rs}, AR:{self.num_ar}, "
...
class VariantConfig:
    ...
    get_pg_config_func: Callable
    get_colls_file_func: Callable
    # Maps repro group names to table group names (e.g. {'513': '1'} means
    # EP group "513" uses intranode timings from table group "1").
    # When set, table entries are remapped to match the repro's group names/sizes.
    get_colls_group_mapping_func: Optional[Callable] = None
...
class CollectiveEstimationParser:
    ...
    def parse_table(file_path: str) -> Dict[Tuple[str, int, str], Dict[int, float]]:
        ...
        if len(lines) < 2:
            logger.warning(
                f"Collective estimation file {file_path} has insufficient data"
            )
            return result
...
class NodeEstimator:
    ...
    def __init__(
        ...
        self.node_names_ests = {
            n.name: est for n, est in nodes_estimations_dict.items()
        }
    ...
    def get_tensor_bytes(node: fx.Node) -> Optional[int]:
        ...
        numel = 1
        for dim in shape:
            numel *= dim  # type: ignore[operator]
    ...
    def get_collective_info(self, node: fx.Node) -> Optional[Tuple[str, int, int, str]]:
        ...
        if "all_gather_into_tensor" in target_str:
            ...
            if len(node.args) >= 2:
                group_size = (
                    self.get_hint(node.args[1])  # type: ignore[arg-type]
                    if hasattr(node.args[1], "node")
                    else node.args[1]
                )
            ...
            if len(node.args) >= 3:
                group_size = (
                    self.get_hint(node.args[2])  # type: ignore[arg-type]
                    if hasattr(node.args[2], "node")
                    else node.args[2]
                )
        ...
        return (collective_name, group_size, tensor_bytes, group_name)  # type: ignore[return-value]
        ...
        if coll_info is not None:
            ...
            if group_size is not None:
                for (
                    ...
                    gs,
                    cn,
                ), size_to_time in self.collective_table.items():
                    if (
                        gs == group_size
                        and cn == collective_name
                        and table_group in node_group_name
                    ):
                        return CollectiveEstimationParser.interpolate_time(
                            size_to_time, size_mb
                        )
            else:
                for (
                    table_group,
...
class ExperimentRunner:
    ...
    def setup_process_groups(self) -> None:
        """Set up fake process groups matching the repro's pg_config."""
        from torch.testing._internal.distributed.fake_pg import FakeStore

        pg_config = self.variant_config.get_pg_config_func()
        world_size = max(info["size"] for info in pg_config.values())
        ...
        # Create all subgroups (new_group auto-registers under generated names)
        pgs = {}
        for name, info in pg_config.items():
        ...
        for name, pg in pgs.items():
            torch._C._distributed_c10d._register_process_group(name, pg)
    ...
    def run_experiment(
        ...
        try:
            ...
            with torch.no_grad():
                ...
                from torch._inductor.fx_passes.overlap_scheduling import (  # type: ignore[attr-defined]
    ...
    def _create_estimator(
        self, nodes_estimations_dict: Dict[fx.Node, float], colls_file_path: str
    ) -> NodeEstimator:
        """Create a node estimator with collective table."""
        raw_table = CollectiveEstimationParser.parse_table(colls_file_path)

        mapping_func = self.variant_config.get_colls_group_mapping_func
        if mapping_func is not None:
            # Remap table entries: replace table group names/sizes with repro's
            pg_config = self.variant_config.get_pg_config_func()
            mapping = mapping_func()
            collective_table = {}
            for repro_group, table_group in mapping.items():
                repro_size = pg_config[repro_group]["size"]
                for (tg, gs, cn), timings in raw_table.items():
                    if tg == table_group:
                        collective_table[(repro_group, repro_size, cn)] = timings
        else:
            collective_table = raw_table
...
def get_variant_configs() -> Dict[str, VariantConfig]:
    ...
    from repro_dsv3_bw_64 import Repro as Repro_dsv3_bw_64
    from repro_dsv3_bw_64 import get_colls_estimations_file as get_colls_file_dsv3_bw_64
    from repro_dsv3_bw_64 import get_colls_group_mapping as get_colls_mapping_dsv3_bw_64
    from repro_dsv3_bw_64 import get_pg_config as get_pg_config_dsv3_bw_64
    from repro_dsv3_bw_64 import load_args as load_args_dsv3_bw_64
    from repro_dsv3_bw_128 import Repro as Repro_dsv3_bw_128
    from repro_dsv3_bw_128 import (
        get_colls_estimations_file as get_colls_file_dsv3_bw_128,
    )
    from repro_dsv3_bw_128 import (
        get_colls_group_mapping as get_colls_mapping_dsv3_bw_128,
    )
    from repro_dsv3_bw_128 import get_pg_config as get_pg_config_dsv3_bw_128
    from repro_dsv3_bw_128 import load_args as load_args_dsv3_bw_128
    from repro_dsv3_fw_64 import Repro as Repro_dsv3_fw_64
    from repro_dsv3_fw_64 import get_colls_estimations_file as get_colls_file_dsv3_fw_64
    from repro_dsv3_fw_64 import get_colls_group_mapping as get_colls_mapping_dsv3_fw_64
    from repro_dsv3_fw_64 import get_pg_config as get_pg_config_dsv3_fw_64
    from repro_dsv3_fw_64 import load_args as load_args_dsv3_fw_64
    from repro_dsv3_fw_128 import Repro as Repro_dsv3_fw_128
    from repro_dsv3_fw_128 import (
        get_colls_estimations_file as get_colls_file_dsv3_fw_128,
    )
    from repro_dsv3_fw_128 import (
        get_colls_group_mapping as get_colls_mapping_dsv3_fw_128,
    )
    from repro_dsv3_fw_128 import get_pg_config as get_pg_config_dsv3_fw_128
    from repro_dsv3_fw_128 import load_args as load_args_dsv3_fw_128
    from repro_llama3_8b_bw_64_1d import Repro as Repro_bw_64_1d
    from repro_llama3_8b_bw_64_1d import (
        get_colls_estimations_file as get_colls_file_bw_64_1d,
    )
    from repro_llama3_8b_bw_64_1d import get_pg_config as get_pg_config_bw_64_1d
    from repro_llama3_8b_bw_64_1d import load_args as load_args_bw_64_1d
    from repro_llama3_8b_bw_64_2d import Repro as Repro_bw_64_2d
    from repro_llama3_8b_bw_64_2d import (
        get_colls_estimations_file as get_colls_file_bw_64_2d,
    )
    from repro_llama3_8b_bw_64_2d import get_pg_config as get_pg_config_bw_64_2d
    from repro_llama3_8b_bw_64_2d import load_args as load_args_bw_64_2d
    from repro_llama3_8b_bw_256_1d import Repro as Repro_bw_256_1d
    from repro_llama3_8b_bw_256_1d import (
        get_colls_estimations_file as get_colls_file_bw_256_1d,
    )
    from repro_llama3_8b_bw_256_1d import get_pg_config as get_pg_config_bw_256_1d
    from repro_llama3_8b_bw_256_1d import load_args as load_args_bw_256_1d
    from repro_llama3_8b_bw_256_2d import Repro as Repro_bw_256_2d
    from repro_llama3_8b_bw_256_2d import (
        get_colls_estimations_file as get_colls_file_bw_256_2d,
    )
    from repro_llama3_8b_bw_256_2d import get_pg_config as get_pg_config_bw_256_2d
    from repro_llama3_8b_bw_256_2d import load_args as load_args_bw_256_2d
    from repro_llama3_8b_fw_64_1d import Repro as Repro_fw_64_1d
    from repro_llama3_8b_fw_64_1d import (
        get_colls_estimations_file as get_colls_file_fw_64_1d,
    )
    from repro_llama3_8b_fw_64_1d import get_pg_config as get_pg_config_fw_64_1d
    from repro_llama3_8b_fw_64_1d import load_args as load_args_fw_64_1d
    from repro_llama3_8b_fw_64_2d import Repro as Repro_fw_64_2d
    from repro_llama3_8b_fw_64_2d import (
        get_colls_estimations_file as get_colls_file_fw_64_2d,
    )
    from repro_llama3_8b_fw_64_2d import get_pg_config as get_pg_config_fw_64_2d
    from repro_llama3_8b_fw_64_2d import load_args as load_args_fw_64_2d
    from repro_llama3_8b_fw_256_1d import Repro as Repro_fw_256_1d
    from repro_llama3_8b_fw_256_1d import (
        get_colls_estimations_file as get_colls_file_fw_256_1d,
    )
    from repro_llama3_8b_fw_256_1d import get_pg_config as get_pg_config_fw_256_1d
    from repro_llama3_8b_fw_256_1d import load_args as load_args_fw_256_1d
    from repro_llama3_8b_fw_256_2d import Repro as Repro_fw_256_2d
    from repro_llama3_8b_fw_256_2d import (
        get_colls_estimations_file as get_colls_file_fw_256_2d,
    )
    from repro_llama3_8b_fw_256_2d import get_pg_config as get_pg_config_fw_256_2d
    from repro_llama3_8b_fw_256_2d import load_args as load_args_fw_256_2d

    return {
        "llama3_8b_bw_256_2d": VariantConfig(
            Repro_bw_256_2d,
            load_args_bw_256_2d,
            get_pg_config_bw_256_2d,
            get_colls_file_bw_256_2d,
        ),
        "llama3_8b_bw_256_1d": VariantConfig(
            Repro_bw_256_1d,
            load_args_bw_256_1d,
            get_pg_config_bw_256_1d,
            get_colls_file_bw_256_1d,
        ),
        "llama3_8b_bw_64_2d": VariantConfig(
            Repro_bw_64_2d,
            load_args_bw_64_2d,
            get_pg_config_bw_64_2d,
            get_colls_file_bw_64_2d,
        ),
        "llama3_8b_bw_64_1d": VariantConfig(
            Repro_bw_64_1d,
            load_args_bw_64_1d,
            get_pg_config_bw_64_1d,
            get_colls_file_bw_64_1d,
        ),
        "llama3_8b_fw_256_2d": VariantConfig(
            Repro_fw_256_2d,
            load_args_fw_256_2d,
            get_pg_config_fw_256_2d,
            get_colls_file_fw_256_2d,
        ),
        "llama3_8b_fw_256_1d": VariantConfig(
            Repro_fw_256_1d,
            load_args_fw_256_1d,
            get_pg_config_fw_256_1d,
            get_colls_file_fw_256_1d,
        ),
        "llama3_8b_fw_64_2d": VariantConfig(
            Repro_fw_64_2d,
            load_args_fw_64_2d,
            get_pg_config_fw_64_2d,
            get_colls_file_fw_64_2d,
        ),
        "llama3_8b_fw_64_1d": VariantConfig(
            Repro_fw_64_1d,
            load_args_fw_64_1d,
            get_pg_config_fw_64_1d,
            get_colls_file_fw_64_1d,
        ),
        "dsv3_fw_64": VariantConfig(
            Repro_dsv3_fw_64,
            load_args_dsv3_fw_64,
            get_pg_config_dsv3_fw_64,
            get_colls_file_dsv3_fw_64,
            get_colls_mapping_dsv3_fw_64,
        ),
        "dsv3_bw_64": VariantConfig(
            Repro_dsv3_bw_64,
            load_args_dsv3_bw_64,
            get_pg_config_dsv3_bw_64,
            get_colls_file_dsv3_bw_64,
            get_colls_mapping_dsv3_bw_64,
        ),
        "dsv3_fw_128": VariantConfig(
            Repro_dsv3_fw_128,
            load_args_dsv3_fw_128,
            get_pg_config_dsv3_fw_128,
            get_colls_file_dsv3_fw_128,
            get_colls_mapping_dsv3_fw_128,
        ),
        "dsv3_bw_128": VariantConfig(
            Repro_dsv3_bw_128,
            load_args_dsv3_bw_128,
            get_pg_config_dsv3_bw_128,
            get_colls_file_dsv3_bw_128,
...
Examples:
  ...
  python run.py --variant llama3_8b_bw_256_2d
""",
    )
...
def main() -> None:
    ...
    try:
        logger.info(
            f"Running overlap scheduling experiment for variant: {args.variant}"
        )
        ...
        if stats_before.runtime > 0:
            improvement = (
                (stats_before.runtime - stats_after.runtime) / stats_before.runtime
            ) * 100
