...
class ApplyShardingInterpreter(torch.fx.Interpreter):
    def __init__(self, module, sharding_placement):
        ...
        self.param_and_grad_nodes = list(
            get_param_and_grad_nodes(module.graph).values()
        )
        self.param_and_single_user = {}
        self.grad_and_single_inputs = {}
        for param, grad in self.param_and_grad_nodes:
            ...
            for p in p_chain:
                self.param_and_single_user[p] = param
            ...
            for p in reversed(g_chain):
                self.grad_and_single_inputs[p] = grad

        combined = {**self.param_and_single_user, **self.grad_and_single_inputs}
        self.redist_map = {}
        mesh_dim = None
        for node, tgt_node in combined.items():

            d = self.get_src_tgt_placements(node)
            if d:
                # self.redist_map[node] = d
                self.redist_map[tgt_node] = (node, d)
                if mesh_dim is None:
                    plc = list(d.values())[0][0]
                    mesh_dim = len(plc)

        self.param_map = {p: g for p, g in self.param_and_grad_nodes}
        aligned_pg = []
        for node in self.redist_map.keys():
            # just allow for arbitrary execution order if both param and grad
            # are in the map
            if node in self.param_map:
                grad_node = self.param_map[node]
                if grad_node in self.redist_map:
                    aligned_pg.append(
                        (
                            node,
                            grad_node,
                            list(self.redist_map[node][1].values())[0],
                            list(self.redist_map[grad_node][1].values())[0],
                        )
                    )
        ...
        for (
            ...
            if node_plc != grad_tgt_plc:
                ...
                continue
            src_tgt_input = (
                self.redist_map[node][0],
                list(self.redist_map[node][1].keys())[0],
            )
            src_tgt_grad = (
                self.redist_map[grad_node][0],
                list(self.redist_map[grad_node][1].keys())[0],
            )
    ...
    def get_src_tgt_placements(self, node):
