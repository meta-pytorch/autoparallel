...
def get_placement_options(mesh, op, specs, user_args, user_kwargs):
    ...
    breakpoint()
    """
%alias_default_6 : [num_users=1] = call_function[target=torch.ops.aten.alias.default](args = (%primals_7,), kwargs = {})
>>> mesh
DeviceMesh((dp=8, tp=8, cp=4), device: 'cuda', stride: (32, 4, 1))
>>> op
<OpOverload(op='aten.alias', overload='default')>
>>> specs
(<torch.distributed.tensor._op_schema.OpStrategy object at 0x7f0060f60350>,)
>>> user_args
(FakeTensor(..., device='cuda:0', size=(64, 256, 6144)),)
>>> user_kwargs
{}
    """
...
def get_local_map_placement_option(
    ...
    # out_specs requires to sometimes flatten out_specs
    if len(out_specs) == 1:
