...
def train(
    *script_args: str,
    ...
    name: str = "muse-glimmer-30b-native-inductor-s4096",
    ...
    if len(script_args) != 6:
        raise ValueError(
            "Expected: {gate|formal} WORLD_SIZE 2 single_native sequential REPLICATE"
        )
    mode, world_size_text, tp_degree_text, matrix, order, replicate = script_args
    if (
        mode not in {"gate", "formal"}
        or matrix != "single_native"
        or order != "sequential"
    ):
    ...
    role.name = "training"
    num_phases = 1 if mode == "gate" else 2
