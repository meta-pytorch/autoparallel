    ...
    for directory, dirnames, filenames in os.walk(root, followlinks=False):
        ...
        for filename in filenames:
            ...
            if relative == "METADATA" or relative.endswith(".CHECKSUMS"):
                continue
            value = (
                path.resolve().read_bytes() if path.is_symlink() else path.read_bytes()
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--package-kind",
        choices=("dryrun", "formal_dryrun", "submitted", "formal_submitted"),
        required=True,
    )
    ...
    comparisons = {
        ...
        "torchtitan": compare(
            task_root / "source/torchtitan", additional / "torchtitan"
        ),
