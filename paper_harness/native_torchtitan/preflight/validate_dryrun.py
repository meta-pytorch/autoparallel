...
def main() -> None:
    ...
    phase_count = 1 if args.mode == "gate" else 2
    ...
    check(
        "config",
        env.get("CONFIG") == "muse_glimmer_30b_sdpa_c4_torchtitan_4x2",
    )
    check(
        "pythonpath_order",
    ...
    check(
        "performance_phases",
        "01_per_gpu_bs1_torchtitan_native_inductor" in runner_text,
    )
    check(
        "trace_phases",
        "11_trace_per_gpu_bs1_torchtitan_native_inductor" in runner_text,
    )
    check(
        "single_native_config",
        runner_text.count("muse_glimmer_30b_sdpa_c4_torchtitan_4x2") == 2
        and "graph_trainer_muse_glimmer_30b_sdpa_c4_4x2" not in runner_text
        and "graph_trainer_muse_glimmer_30b_sdpa_c4_autoparallel_4x2"
        not in runner_text,
    )
    ...
    check(
        ...
        "local_batch_sizes=(2)" in runner_text
        and "local_batch_sizes+=(2)" in runner_text
    ...
    check(
        "native_inductor_compile",
        "--compile.enable" in runner_text
        and "--compile.backend inductor" in runner_text
        and "--compile.debug-graph-passes" not in runner_text,
    )
    check(
    ...
    check(
        ...
        and '[[ -e "${RUN_ROOT}/runtime/${phase}.completed" ]]' in runner_text
        and "unexpected_failure=0" not in runner_text
