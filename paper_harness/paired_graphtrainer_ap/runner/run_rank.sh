...
if [[ $# != 6 ]] || [[ $1 != gate && $1 != formal ]] || [[ $4 != two_arm ]]; then
    echo "Usage: $0 {gate|formal} 16 2 two_arm sequential REPLICATE" >&2
    exit 2
fi
run_mode=$1
world_size=$2
...
export RUN_ROOT=${DUMP_DIR}/muse_glimmer_30b_sdpa_fix_sac_aligned_lb2_s4096_${node_count}x8_${run_mode}_r${replicate}_v${MAST_HPC_JOB_VERSION}_a${MAST_HPC_JOB_ATTEMPT_INDEX}
...
phase_kinds=(performance performance)
local_batch_sizes=(2 2)
...
run_phase() {
    ...
    if [[ ${run_mode} == gate ]]; then
        ...
        debug_args=(--compile.debug-graph-passes)
    ...
    [[ ! -e "${RUN_ROOT}/runtime/${phase}.failed" ]]
}
