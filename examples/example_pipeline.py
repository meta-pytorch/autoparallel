...
# Pipeline helpers copied from torchtitan
...
assert "WORLD_SIZE" in os.environ, "run with torchrun --nproc-per-node 4 or 8"
world_size = int(os.getenv("WORLD_SIZE"))
pp_degree = 2
world_mesh = torch.distributed.device_mesh.init_device_mesh(
    "cuda",
    (pp_degree, world_size // pp_degree),
    mesh_dim_names=(
        "pp",
        "dp",
    ),
...
spmd_mesh = world_mesh["dp"]

global_batch_size = 4 * world_mesh.shape[0]
dp_degree = world_mesh.size(1)
...
def model_fn():
    ...
    # Very simplified / hardcoded version of split util in torchtitan, for the case of pp=2 n_layers=4
    if pp_rank == 0:
        del m.layers["2"]
        del m.layers["3"]
        m.norm = None
...
def input_fn(runtime=False):
    # batch_size = microbatch_size if runtime else spmd_batch_size
    # note: don't use microbatch_size at runtime, bc pipeline runtime will actually split the mb's
    batch_size = spmd_batch_size
    if pp_rank == 0:
        x = torch.randint(0, vocab_size, (batch_size, seqlen), device=device)
    else:
        x = torch.randn(
            (batch_size, seqlen, model.model_args.dim),
            device=device,
...
# parallelize the model
with AutoParallel(
    model, input_fn, spmd_mesh, mp_policy, compile=True, repeated_subgraphs=True
) as autop:
    autop.add_parameter_memory_constraint(low=None, high=None)

    x_sharding = (Shard(0),) + (Replicate(),) * (spmd_mesh.ndim - 1)
    out_sharding = x_sharding
    if use_vocab_parallel:
        # add vocab parallel constraint
        assert spmd_mesh.ndim == 2, "Only 2d mesh supported here"
